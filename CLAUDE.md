...
### Step 2.5: 在庫チェック（必須・自動中断あり）

⚠️ **在庫確認の厳格ルール（トラブル防止）**

Keepaから取得した在庫情報を確認し、以下の条件を満たさない場合は**出品を中断**する：

```
在庫判定ルール:
1. stock_count >= 1 → ✅ 出品OK（具体的な在庫数がある）
2. stock_count = -1 → ❌ 出品中断（数量不明、購入不可の可能性あり）
3. stock_count = 0  → ❌ 出品中断（在庫切れ）
4. stock_available = false → ❌ 出品中断（在庫なし）

中断時のメッセージ例:
「⚠️ この商品は在庫が不安定です（stock_count: -1）。
Amazonページを確認してカートに入れられるか確認してください。
購入可能であれば、再度URLを入力してください。」
```

**重要事項:**
- `skip_keepa_check: true` は使用禁止（必ず在庫チェックを行う）
- stock_count = -1（数量不明）の商品は、Keepaが「在庫あり」と判定していても実際には購入不可の場合がある
- 在庫数が具体的に表示されている商品のみ出品対象とする
